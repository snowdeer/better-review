...
export abstract class RulesBuilder<TTokenInfo extends {}, TState> {
  ...
  addRule(matcher: RegExp, tokenInfo: TTokenInfo | (TTokenInfo | null)[]): void;
  addRule(
    ...args:
    ...
    if (args.length === 1) {
      ...
      const x: RegExpRule<TTokenInfo, TState>["getTokens"] = Array.isArray(
        tokenInfo
      )
        ? (match, state) => {
            ...
            if (offset !== match[0].length) {
              throw new Error("Groups must partionize the matched string!");
            }
            return result;
          }
        : (match, state) => [
  ...
  match(
    ...
    for (const r of this.rules) {
      ...
      if (m.matches) {
        ...
        if (m.offsetOfNextPossibleMatch < smallestOffsetOfNextPossibleMatch) {
          smallestOffsetOfNextPossibleMatch = m.offsetOfNextPossibleMatch;
        }
      }
    }
